...
export default function LoginSignup() {
  ...
  const handleSignupSubmit = async (e) => {
    ...
    try {
      ...
      if (token && user) {
        ...
        // ✅ Navigate to sendcall with welcome popup state
        navigate("/sendcall", {
          state: {
            showWelcome: true,
            trialMinutes: user?.minute || 10,
          },
        });
      } else {
        toast.success("Signup successful! Please log in.");
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-center min-h-screen px-4 py-8">
      <div className="w-full sm:w-2/3 md:w-1/3 flex justify-center items-center h-auto sm:h-[50vh] md:h-[60vh] lg:h-[70vh] p-4">
        <img src="/Richa.png" alt="Login Visual" className="w-auto h-full max-h-[70vh] object-cover rounded-3xl shadow-lg" />
      </div>

      <div className="w-full md:w-1/2 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
          <div className="flex">
            <button onClick={() => handleTabClick("login")} className={`w-1/2 py-3 rounded-tl-2xl text-lg font-bold ${activeTab === "login" ? "bg-blue-600 text-white" : "bg-blue-200 text-gray-700"}`}>
              Login
            </button>
            <button onClick={() => handleTabClick("signup")} className={`w-1/2 py-3 rounded-tr-2xl text-lg font-bold ${activeTab === "signup" ? "bg-blue-600 text-white" : "bg-blue-200 text-gray-700"}`}>
              Signup
            </button>
          </div>
