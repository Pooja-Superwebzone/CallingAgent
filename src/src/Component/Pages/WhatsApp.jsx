...
import { Paperclip, Smile, Send, RefreshCw, ArrowLeft } from "lucide-react";
...
const WhatsApp = () => {
  ...
  const handleSendMessage = async () => {
    ...
    const tempMsg = {
      ...
      ...(selectedFile && { file: selectedFile, fileName: selectedFile.name })
    };
    ...
    try {
      await sendWhatsappTextMessage({ to: rawPhone, message: messageToSend });
    } catch (error) {
  ...
  return (
    <div className="relative flex flex-col md:flex-row h-screen w-full">
      ...
      <aside className={`fixed md:static top-0 left-0 h-full bg-gray-50 z-40 border-r border-gray-200 w-72 transform transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        ...
        <div className="hidden md:block p-4 border-b border-[#cdd1d6]">
        ...
        {selectedChat ? (
          <>
            <div className="flex items-center justify-between p-4 border-b border-[#cdd1d6] bg-white">
              <div className="flex items-center gap-3">
                <button onClick={() => navigate(-1)} className="text-gray-500 hover:text-black md:hidden">
                  <ArrowLeft size={20} />
                </button>
